...
| `@carddemo/batch-readers` | `CBACT01C`, `CBACT02C`, `CBACT03C`, `CBCUS01C` | Batch readers for the account, card, card xref and customer masters: each is a function plus a CLI entry point that displays the records it reads |
...
## Running the batch programs

```bash
npm run cbact01c                       # app/data/ASCII/acctdata.txt
npm run cbact02c -- /path/to/carddata.txt
```

The first argument replaces the DD statement in the JCL. A file status other
than `00` on open, read or close displays the COBOL diagnostics (`FILE STATUS
IS: NNNN…`, `ABENDING PROGRAM`) and exits non-zero, standing in for
`CALL 'CEE3ABD'`.

`CBACT01C` additionally writes the OUTFILE, ARRYFILE and VBRCFILE extracts via
the `COBDATFT` assembler date routine; that side of the program is not ported
yet, so only its reader and display behaviour is reproduced.
