...
| T11 | `MOVE WS-DATE-TO-TEST TO WS-DATE` copies the halfword-prefixed group into a `PIC X(10)` field, so the two binary length bytes lead the echoed date in the result message; the 88-level for a zero feedback code is named `FC-INVALID-DATE` but yields `'Date is valid'` | `app/cbl/CSUTLDTC.cbl:122`, `app/cbl/CSUTLDTC.cbl:52`; `app/cbl/CSUTLDTC.cbl:62`, `app/cbl/CSUTLDTC.cbl:129-130` | `WS-DATE X(10)` | group → alphanumeric, left-justified | none; the message is informational | Inferred |
