...
| 350-byte `TRAN-RECORD` / 430-byte `REJECT-RECORD` built by MOVEs | `TransactionLayout.encodeInto`; the reader emits `DailyTransaction(decoded, rawImage)` and `RejectRecordLayout.encode(rawImage, reason)` does a group MOVE of the bytes | Byte-exact lengths; reject copies the **original input image** (including the `X(20)` filler the layout does not decode), not a re-encoded one. |
| `9000-*-CLOSE` paragraphs; a CLOSE/WRITE failure abends 999 | Persisting the four outputs is its own tasklet step (`closeDatasets`) after the chunk step, not a step listener | A write failure fails the step and the job. Spring Batch logs but does not propagate listener exceptions, so a listener would have reported success with partial datasets. Writes are still not atomic across the four files (see PR discussion / OQ-07). |
| `RETURN-CODE 4` when rejects > 0 | `closeDatasets` returns `ExitStatus("COMPLETED_WITH_REJECTS")` only after all outputs are written; `ExitCodeMapper` maps it to process exit 4 | Same scheduler contract. Not verified against a real launcher (see §5.3). |
...
| `0550-DATEPARM-READ` status `10` → `END-OF-FILE = 'Y'` | Empty DATEPARM → empty TRANREPT written, job `COMPLETED` | Same as the COBOL (lines 235-236): no report body, normal end. Other DATEPARM errors still fail the job. |
