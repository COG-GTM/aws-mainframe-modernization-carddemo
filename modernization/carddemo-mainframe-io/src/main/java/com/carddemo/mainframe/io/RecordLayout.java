 ...
 * {@link com.carddemo.mainframe.io.layout.CardDemoLayouts}) and then used to read and write
...
public final class RecordLayout {
    ...
    public enum FieldKind {
        ...
        PACKED_DECIMAL,
        /** An embedded record image copied byte for byte, without character conversion. */
        RAW
    }
    ...
    /**
     * Reads a field as raw bytes, without character conversion.
     *
     * <p>Used where COBOL moves one whole record area into another (for example the daily
     * transaction embedded in a rejected-transaction record), so that bytes the copybook does not
     * describe survive the copy unaltered.
     */
    public byte[] raw(byte[] record, String fieldName) {
        FieldDefinition definition = field(fieldName);
        return java.util.Arrays.copyOfRange(record, definition.offset(),
                definition.offset() + definition.byteLength());
    }

    /** Writes a raw byte field, which must be exactly the declared field width. */
    public void putRaw(byte[] record, String fieldName, byte[] value) {
        FieldDefinition definition = field(fieldName);
        if (value.length != definition.byteLength()) {
            throw new IllegalArgumentException("Field '" + fieldName + "' of layout " + name
                    + " is " + definition.byteLength() + " bytes, got " + value.length);
        }
        System.arraycopy(value, 0, record, definition.offset(), definition.byteLength());
    }
    ...
    public BigDecimal decimal(byte[] record, String fieldName) {
        ...
        return switch (definition.kind()) {
            ...
            case PACKED_DECIMAL -> PackedDecimalCodec.decode(
                    record, definition.offset(), definition.digits(), definition.scale());
            case ALPHANUMERIC, RAW -> throw new IllegalArgumentException(
                    "Field '" + fieldName + "' of layout " + name + " is not numeric");
        };
    }
    ...
    public static final class Builder {
        ...
        /** Declares a {@code PIC X(length)} field copied as raw bytes rather than as text. */
        public Builder raw(String fieldName, int length) {
            return add(new FieldDefinition(fieldName, FieldKind.RAW, offset, length, 0, 0, false));
        }
